...
SERIAL_ECHOPAIR("\nCASE_LIGHT_PIN : ", (int) CASE_LIGHT_PIN );
SERIAL_ECHOPAIR("\nNUM_SERVOS : ", (int) NUM_SERVOS);
SERIAL_ECHOPAIR("\nSERVO0_PIN : ", (int) SERVO0_PIN);
SERIAL_ECHOPAIR("\nZ_PROBE_SERVO_NR : ", (int) Z_PROBE_SERVO_NR);

//SERIAL_ECHOPAIR("\nHAS_SERVO_0 : ", (int) HAS_SERVO_0);
SERIAL_ECHOPAIR("\nUSEABLE_HARDWARE_PWM(CASE_LIGHT_PIN) : ", (int) USEABLE_HARDWARE_PWM(CASE_LIGHT_PIN) );
SERIAL_ECHOPAIR("\nPWM_CHK(p) : ", (int) PWM_CHK(CASE_LIGHT_PIN) );
SERIAL_ECHOPAIR("\nPWM_CHK_HEATER(p) : ", (int) PWM_CHK_HEATER(CASE_LIGHT_PIN));
SERIAL_ECHOPAIR("\nPWM_CHK_SERVO(p) : ", (int) PWM_CHK_SERVO(CASE_LIGHT_PIN));
SERIAL_ECHOPAIR("\nPWM_CHK_MOTOR_CURRENT(p) : ", (int) PWM_CHK_MOTOR_CURRENT(CASE_LIGHT_PIN));
SERIAL_ECHOPAIR("\nPWM_CHK_FAN_A(p) : ", PWM_CHK_FAN_A(CASE_LIGHT_PIN));
SERIAL_ECHOPAIR("\nPWM_CHK_FAN_B(p) : ", PWM_CHK_FAN_B(CASE_LIGHT_PIN));
SERIAL_ECHO("\n");

   pinMode(5,OUTPUT); //First we set the pin to output and use the analogWrite command to initialise the PWM 
   analogWrite(5,1); 

//   OCR3B = 128; //Now we can change the duty cycle using just the register 
//   OCR3B = 4; 
//5	OCR3A
